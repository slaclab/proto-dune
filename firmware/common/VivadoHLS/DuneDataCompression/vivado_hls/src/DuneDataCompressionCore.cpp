...
struct ReadFrame
{
      ...
      statusw(0)
  {
   ...
   ReadStatus_t            statusw;  /*!< Status consumed in the write     */
                             ...
                             MonitorModule   &monitor)
{
   ...
   #pragma HLS DATA_PACK variable=monitor.read.nWibErrs
   #pragma HLS DATA_PACK variable=monitor.read.nStates
   #pragma HLS DATA_PACK variable=monitor.read.nFrameErrs
   #pragma HLS DATA_PACK variable=monitor.write
   //#pragma HLS DATA_PACK variable=cfg


   static ReadFrame     frame;
   ////#pragma HLS STREAM   variable=frame.d off
   ///#pragma HLS STREAM   variable=frame.status
                             ...
                             MonitorModule   &monitor)
{
   ...
   #pragma HLS DATA_PACK variable=cfg
    ...
    static ReadFrame     frame;
    #pragma HLS STREAM   variable=frame.d
    #pragma HLS STREAM   variable=frame.status
...
#pragma HLS INLINE off
   ...
   ///read_colddata_hdrs (status, frame.d + 2,  axis, ctx.hdrs + 2, ctx.cvtCnt0, 0);
   ///read_colddata_data (status, frame.d + 4,  axis, false);

   ///read_colddata_hdrs (status, frame.d + 16, axis, ctx.hdrs + 4, ctx.cvtCnt1, 8);
   ///read_colddata_data (status, frame.d + 18, axis, true);
...
#else
   {
      ...
      // Capture the state of Start Of Frame bit
      sof = in.user.test (static_cast<uint8_t>(AxisUserFirst::Sof));
      eof = in.last.test (static_cast<uint8_t>(AxisLast::     Eof));

      // Check if
      //   1) Start of Frame is missing on the first sample
      //   2) End   of Frame is present on the first sample
      if (!sof) status.set (ReadFrame::ErrSofM);
      if ( eof) status.set (ReadFrame::ErrEofU);


      // !!! KLUDGE !!!
      // Seed the Wib Id with the first seen
      // This is not a great idea
      if (First)
      {
         First     = false;
         ctx.wibId = WibFrame::id (in.data);
      }

      //  Check that all the fields are as expected
      eval_wib0 (status, in.data, ctx.wibId);
   }
#endif
   ...
   for (int idx = 16; idx < 30; idx++)
   {
      ...
      {
         // Check
         //   End Of Frame bit is not present on any other sample
         if ( eof) status.set (ReadFrame::ErrEofU);
      }
   }
   }
   ...
   AxisIn_t in = axis.read ();
    ...
    //  Check that all the fields are as expected
    eval_wib1 (status, in.data, ctx.hdrs[1]);

    // Capture the header and update for use as the seed for the next frame
    ctx.hdrs[1] = in.data + 25;
    frame.d [1] = in.data;
   ...
   // Flush frames in active packets are.  It is up to the receiver to dispose of these.
                        ...
                        MonitorRead     &lcl)
{
   ...
   update_read (gbl,  gbl, cfg, status);
                         ...
                         ReadStatus_t     status)
{
   ...
   ///#pragma HLS RESET variable=lcl off
...
static void write_frame (AxisOut           &axis,
                         ...
                         MonitorWrite       &lcl)
{
   static WriteContext    ctx;
   static ReadStatus_t summary;
   ReadStatus_t         status = frame.statusw;

   // Count the number of frames to be written
    // A good frame is one with SOF and EOF set only on the proper words and no Eof Err
    // Flush frames with nothing in the packet are not promoted.
    // Flush frames in active packets are.  It is up to the receiver to dispose of these.
    bool writeIt = ReadFrame::isGoodFrame (status) &&
                     ! ((ctx.fdx == 0) && status.test (static_cast<unsigned>(ReadFrame::Flush)));

    ////printf ("Write:Status %d;%8.8x\n", writeIt, frame.statusw.to_uint ());
   ...
   if (ctx.odx == 0)
   {
       prologue (axis, ctx.odx, writeIt, 0);
       if (writeIt) summary = frame.statusw;
       else         summary =      0;
   }
   else
   {
   ...
   for (int idx = 0; idx < sizeof (WibFrame)/sizeof (uint64_t); idx++)
   {
      ...
      d = frame.d[idx];
      //d = frame.d.read ();
      commit (axis, ctx.odx, writeIt, d, 0, 0);
   }

   if (writeIt) { lcl.npromoted += 1;  ctx.fdx += 1; }
   else         { lcl.ndropped  += 1; }
   ...
   // --------------------------------------------
   bool output;
   if (ctx.odx && ((ctx.fdx & 0x3ff)  == 0))
   {
      printf ("Outputting packet frame.status = %8.8x ctx.fdx = %8.8x\n",
              frame.status.to_uint(), lcl.npromoted);
      epilogue (axis, summary, ctx.odx);
