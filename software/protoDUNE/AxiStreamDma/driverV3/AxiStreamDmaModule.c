...
int AxiStreamDmaProcRead(char *buf, char **start, off_t offset, __s32 count, __s32 *eof, void *data) {
   __s32 len;
   __s32 x;
   __s32 cnt;
   __s32 max;
   ...
   __s32 avg;
	...
	struct AxiStreamDmaDevice *dev = (struct AxiStreamDmaDevice *)data;
   len = 0;
...
static int AxiStreamDmaProbe(struct platform_device *pdev) {
   ...
   // Setup /proc
   create_proc_read_entry(dev->devName,0,NULL,AxiStreamDmaProcRead,dev);
