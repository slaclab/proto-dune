...
int32_t McsRead::read (McsReadData *mem) {
   ...
   // 2016.09.22 -- jjr
   // -----------------
   // Added initialization of status, but this just keeps the compiler happy
   // The real issue is that what should it be set to.
   // As long as one is not at the endOfFile, status gets set, but
   // if you enter the routine with endOfFile set, status is not set.
   // -----------------------------------------------------------------------
   int32_t status = 0;
