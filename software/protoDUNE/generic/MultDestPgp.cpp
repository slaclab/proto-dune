...
//! Constructor
MultDestPgp::MultDestPgp (string path) : MultDest(512) { 
   path_ = path;
}

//! Deconstructor
MultDestPgp::~MultDestPgp() { 
   this->close();
}

//! Open link
void MultDestPgp::open ( uint32_t idx, uint32_t maxRxTx ) {
   stringstream tmp;
   ...
   if ( fd_ < 0 ) {
      tmp.str("");
      tmp << "MultDestPgp::open -> Could Not Open PGP path " << path_;
      throw tmp.str();
   }

   if ( debug_ ) 
      cout << "MultDestPgp::open -> Opened pgp device " << path_
           << ", Fd=" << dec << fd_ << endl;
...
//! Transmit data.
int32_t MultDestPgp::transmit ( MultType type, void *ptr, uint32_t size, uint32_t context, uint32_t config ) {
   uint32_t   lane;
   uint32_t   vc;
   Register * reg;
   Command  * cmd;
   ...
   switch ( type ) {
      ...
      default:
         ...
         // MultiDestPgpG3.cpp with the same problem.
         // -----------------------------------------------------------
         return -1;
...
// Receive data
int32_t MultDestPgp::receive ( MultType *type, void **ptr, uint32_t *context ) {
   int32_t  ret;
   uint32_t lane;
   uint32_t vc;
   uint32_t eofe;
