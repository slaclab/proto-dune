      ...
      if ( smem_->usReqCount != smem_->usAckCount ) {
         ...
         // Reformat header for register rx
            ...
            // Unexpected frame
               ...
               unexpCount_++;
               if ( debug_ ) {
                  cout << "SimLinkByte::rxHandler -> Unexpected frame received"
...
void SimLinkByte::ioHandler() {
   ...
   while ( runEnable_ && smem_ != NULL ) {
      ...
      // Register TX is pending
         ...
         regBuff_[1] |= (regReqEntry_->address()>>2) & 0x3FFFFFFF;
...
// Constructor
SimLinkByte::SimLinkByte ( ) : CommLink() {
   smem_      = NULL;
   toDisable_ = true;
}

// Deconstructor
SimLinkByte::~SimLinkByte ( ) {
   close();
}

// Open link and start threads
void SimLinkByte::open ( string system, uint32_t id ) {
   int32_t      smemFd;
   char         shmName[200];
   ...
   // Debug
   cout << "SimLinkByte::open -> Using shared memory file " << shmName << endl;
   ...
   if ( smemFd < 0 ) throw string("SimLinkByte::open -> Could Not Open Shared Memory");
   ...
   if((smem_ = (SimLinkMemory *)mmap(0, sizeof(SimLinkMemory),
      ...
      smem_   = NULL;
      if ( smemFd < 0 ) throw string("SimLinkByte::open -> Could Not Map Shared Memory");
   }
#endif
