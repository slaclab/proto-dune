...
// 11/05/2015: created
//-----------------------------------------------------------------------------
#ifndef __UDP_REG_H__
#define __UDP_REG_H__

#include <Device.h>
#include <stdint.h>
using namespace std;

class UdpReg:public Device {        
 public:
    UdpReg ( uint32_t linkConfig, uint32_t baseAddress, uint32_t index, Device *parent, uint32_t addrSize=1 );
    ~UdpReg( ); 
